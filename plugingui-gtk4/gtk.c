...
static GMainContext *gtk4_main_context = NULL;
...
int XemuPluginGuiAPI_iteration ( void )
{
	if (gtkgui_active && is_xemugui_ok) {
		int n = 0;
		while (g_main_context_pending(gtk4_main_context)) {
			g_main_context_iteration(gtk4_main_context, FALSE);
			n++;
		}
		if (n > 0)
...
int XemuPluginGuiAPI_init ( struct xemuplugingui_info_st *info )
{
	...
	gtk4_main_context = g_main_context_default();
	is_xemugui_ok = 1;
...
static void _gtkgui_bgtask_clear ( void ) {
	...
	while (_gtkgui_bgtask_running_status || g_main_context_pending(gtk4_main_context)) {
		g_main_context_iteration(gtk4_main_context, FALSE);
		usleep(10000);
	}
	//xemu_drop_events();
